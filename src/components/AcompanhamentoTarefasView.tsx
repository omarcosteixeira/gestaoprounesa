...
export function AcompanhamentoTarefasView({
  ...
  const filtered = tarefas.filter((t) => {
    ...
    if (isFocusMode) {
      return matchesSearch && (t.status === "Atrasado" || t.status === "Urgente");
    }
  ...
  const countUrgente = tarefas.filter((t) => t.status === "Urgente").length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-lg border border-blue-800/40">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5 w-full md:w-auto">
            ...
            <div className="bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/10 text-center">
              <div className="text-xs text-orange-200 font-semibold">Urgente</div>
              <div className="text-xl font-black text-orange-400 mt-0.5">{countUrgente}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all shadow-sm border ${
              isFocusMode
                ? "bg-orange-600 text-white border-orange-700 ring-4 ring-orange-100 scale-105"
      ...
      {isFocusMode && (
        <div className="bg-orange-50 border border-orange-100 p-4 rounded-2xl flex items-center gap-4 animate-in fade-in slide-in-from-top-2">
          <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 flex-shrink-0">
            <Target size={20} />
          </div>
          <div>
            <h4 className="text-sm font-bold text-orange-900">Visualização de Alta Prioridade</h4>
            <p className="text-xs text-orange-700">
              Exibindo apenas tarefas <strong>Atrasadas</strong> ou <strong>Urgentes</strong> para foco total em pendências críticas.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          {filtered.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className={`bg-white rounded-2xl p-4 shadow-sm border-l-4 ${cfg.border} border-y-slate-100 border-r-slate-100 flex items-center gap-4 hover:shadow-md transition-all`}
              >
                <div className={`w-12 h-12 rounded-xl ${cfg.bg} ${cfg.text} flex items-center justify-center flex-shrink-0`}>
                  <StatusIcon size={24} />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1 text-[11px] font-medium text-slate-500">
