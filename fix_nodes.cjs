...
const regex = /const AvatarPlaceholder = \(\{ name \}\) => \([\s\S]*?\}\);?\s*\}/;
...
function LevelBNode({ f }: { f: any }) {
  return (
    <div className="flex items-center relative z-10 w-64 mb-4 mx-4 bg-white rounded-[2rem] shadow-md pr-4 py-2 mt-4 overflow-hidden">
      ...
      {/* Left Arc spanning top to bottom */}
  ...
  return (
    <div className="flex flex-col items-center relative z-10 w-32 mb-4 mx-2 mt-2">
      <div className="relative w-16 h-16 flex items-center justify-center">
        ...
        {/* Avatar */}
        <div className="w-14 h-14 rounded-full border-2 border-white shadow-sm bg-slate-200 overflow-hidden relative z-10">
          <AvatarPlaceholder name={f.nome} />
        </div>
      </div>
      <div className="text-center mt-3 relative z-10 w-full bg-white/40 rounded-xl p-1 backdrop-blur-sm">
...
fs.writeFileSync('src/components/OrganogramaSmView.tsx', code);
